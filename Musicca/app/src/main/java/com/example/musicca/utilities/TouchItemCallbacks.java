package com.example.musicca.utilities;
...
public class TouchItemCallbacks {
    ...
    public TouchItemCallbacks(RecyclerView.Adapter adapter, Context context) {
        mAdapter = adapter;
        mContext = context;

        // Set the drawables in constructor to avoid having to do so in the onChildDraw
        mAdd = mContext.getDrawable(R.drawable.addsong);
        mLike = mContext.getDrawable(R.drawable.likeiconfilled);
        mRemove = mContext.getDrawable(R.drawable.cancelicon);
    ...
    // adding items from the SearchAdapter
    ...
    // Draw the provided icon on the right side of the ViewHolder
